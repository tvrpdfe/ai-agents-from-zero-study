"""
【案例】天气查询工具：用 @tool 定义可被 LLM 调用的天气接口，请求 OpenWeather Current Weather API 并返回 JSON
...
- Free 计划可用 Current Weather API（/data/2.5/weather），支持城市名直接查询；One Call 3.0/4.0 需单独订阅 One Call by Call，本例不使用。
...
def get_weather(loc: str) -> str:
    ...
    """
    # Step 1. 构建请求 URL
    # Free 计划可用：Current Weather API 2.5（文档 https://openweathermap.org/current）
    # 注意：One Call 3.0/4.0 需单独订阅 One Call by Call，Free 计划不可用
