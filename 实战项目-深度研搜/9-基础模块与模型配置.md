...
项目对应文件：`deepsearch-agents/agent/llm.py`
...
项目对应文件：`deepsearch-agents/agent/prompts.py`
...
如果当前文件是：`deepsearch-agents/agent/prompts.py`

那么：

```python
Path(__file__).parents[0]  # deepsearch-agents/agent
Path(__file__).parents[1]  # deepsearch-agents
```

所以 `parents[1]` 能拿到项目根路径。
...
在 `deepsearch-agents` 根目录运行这个文件：

```bash
uv run agent/prompts.py
```
...
这说明 `prompts.py` 已经成功读到了 `prompt/prompts.yml`，并且把子智能体配置拆成了 Python 字典。
...
```bash
uv run python -m py_compile agent/llm.py agent/prompts.py api/context.py api/monitor.py utils/path_utils.py utils/word_converter.py
```

验证 YAML 是否能被读取：

```bash
uv run python -c "import yaml; yaml.safe_load(open('prompt/prompts.yml', encoding='utf-8')); print('yaml ok')"
```

验证提示词加载：

```bash
uv run python agent/prompts.py
```

验证 `ContextVar` 基础行为：

```bash
uv run python -c "from api.context import set_session_context, get_session_context, set_thread_context, get_thread_context, reset_session_context; s=set_session_context('/tmp/session-a'); t=set_thread_context('thread-a'); assert get_session_context() == '/tmp/session-a'; assert get_thread_context() == 'thread-a'; reset_session_context(s, t); assert get_session_context() is None; assert get_thread_context() is None; print('context ok')"
```

验证 Markdown 转 PDF：

```bash
uv run python -c "from pathlib import Path; from utils.word_converter import convert_md_to_pdf; md=Path('/tmp/test.md'); pdf=Path('/tmp/test.pdf'); md.write_text('# 测试标题\n\n这是中文内容\n\n| 名称 | 值 |\n| --- | --- |\n| A | 100 |\n', encoding='utf-8'); print(convert_md_to_pdf(md, pdf))"
```
